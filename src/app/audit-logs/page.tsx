...
export default function AuditLogsPage() {
  ...
  const [embedUrl, setEmbedUrl] = useState<string>(AUDIT_LOGS_EMBED_URL);
  ...
  useEffect(() => {
    try {
      const url = getEmbedUrlWithDarkMode(AUDIT_LOGS_EMBED_URL, isDarkMode);
      setEmbedUrl(url);
    } catch (error) {
        ...
        <div className="mx-auto max-w-4xl">
          <div className="mb-6">
            ...
            <p className="text-muted-foreground">
              Monitor access decisions and track all activity within your permissions system.
            </p>
          </div>
          ...
          <AuthenticationStep 
            ...
            description="Connect to enable audit logs functionality"
          />

          {/* Step 4: Preview */}
          <PreviewStep
            stepNumber={4}
            title="Audit Logs Element"
            elementId="audit-logs"
            description="View detailed audit logs of all access decisions and permission changes."
            embedUrl={embedUrl}
            envId={environmentId}
            tenantId={tenantId}
            userToken={userJwt}
